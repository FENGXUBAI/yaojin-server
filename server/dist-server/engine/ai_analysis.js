...
exports.CardRecorder = void 0;
exports.analyzeHandStructure = analyzeHandStructure;
...
function scoreSplit(split) {
    ...
    for (const p of split) {
        ...
        if (p.type === 'DOUBLE_SEQUENCE')
            score -= (p.cards.length / 2 - 3) * 10;
    }
    return score;
}
...
function analyzeHandStructure(hand) {
    ...
    const sortedRanks = [...counts.keys()].sort((a, b) => {
        ...
        if (ca !== cb)
            return cb - ca;
        // Value sort
        return cards_1.singleOrder.indexOf(a) - cards_1.singleOrder.indexOf(b); // lower index is higher value
    });
    // Check King Bomb
    ...
    if (big && small) {
        ...
        const p = (0, patterns_1.detectPattern)([big, small]);
        if (p)
    ...
    for (const r of sortedRanks) {
        ...
        if (cards.length === 4) {
            const p = (0, patterns_1.detectPattern)(cards);
            if (p) {
    ...
    for (let i = 0; i < pairRanks.length; i++) {
        ...
        if (currentRun.length === 0) {
            ...
            if ((0, cards_1.nextInRing)(prev) === r) {
                currentRun.push(r);
            }
            else {
    ...
    if (bestRun.length >= 3) {
        ...
        const p = (0, patterns_1.detectPattern)(cardsToRemove);
        if (p) {
    ...
    for (const r of sortedRanks) {
        ...
        if (cards.length >= 2) {
            ...
            const p = (0, patterns_1.detectPattern)(pair);
            if (p) {
                split.push(p);
                for (const c of pair) {
                    const idx = remaining.indexOf(c);
    ...
    for (const c of remaining) {
        const p = (0, patterns_1.detectPattern)([c]);
        if (p)
            split.push(p);
    }
    return split;
...
class CardRecorder {
    constructor() {
        ...
        for (const r of cards_1.singleOrder) {
    ...
    getHigherCandidates(rank, count) {
        const idx = cards_1.singleOrder.indexOf(rank);
        const candidates = [];
        // Iterate over ranks higher than 'rank' (lower index)
        for (let i = 0; i < idx; i++) {
            const r = cards_1.singleOrder[i];
...
exports.CardRecorder = CardRecorder;
