...
const patterns_1 = require("./engine/patterns");
...
app.post('/api/auth/guest', (req, res) => {
    try {
        ...
        const userId = `guest_${crypto_1.default.randomBytes(8).toString('hex')}`;
        const user = {
            id: userId,
...
app.post('/api/auth/wechat', (req, res) => {
    try {
        ...
        // 简化处理：用 code 生成用户ID
        const userId = `wx_${crypto_1.default.createHash('md5').update(code || Date.now().toString()).digest('hex').slice(0, 16)}`;
...
const possiblePaths = [
    ...
    path_1.default.join(__dirname, 'dist'), // old fallback
];
...
function publicizeGameState(state) {
...
function executeBotTurn(room) {
    ...
    if (!player)
        return;
    if (!player.isBot && !player.isTrusteeship)
        return;
    try {
        const action = (0, ai_1.decideBotAction)(state, currentPlayerIdx);
        ...
        // Fallback: pass
        try {
            const nextState = (0, game_1.playTurn)(state, { type: 'pass' });
            room.gameState = nextState;
            io.to(room.id).emit('gameState', publicizeGameState(nextState));
            startTurnTimer(room);
        }
...
function startTurnTimer(room) {
    ...
    const canPlay = (0, game_1.hasValidMove)(hand, compareAgainst);
...
function emitPrivateState(r) {
    ...
    for (let idx = 0; idx < r.players.length; idx++) {
        ...
        if (!target)
            continue;
        const hand = r.gameState.hands[idx];
...
io.on('connection', (socket) => {
    ...
    socket.on('join', ({ room, name, clientKey, lastSfxSeq, lastMvpSeq }) => {
        ...
        // 检查是否有预注册房间需要迁移
        const pending = pendingRooms.get(upperRoom);
        if (!r) {
            // 创建新房间
            r = {
                id: upperRoom,
        ...
        if (!existingByKey && existingByName && existingByName.clientKey && existingByName.clientKey !== resolvedClientKey) {
            socket.emit('error', '该昵称已被占用，请更换昵称或使用原设备重连');
            return;
        }
        // Legacy client (no clientKey) trying to join with an already-claimed name.
        if (!existingByKey && existingByName && existingByName.clientKey && !clientKey) {
    ...
    socket.on('setMvpSound', ({ room, sound }) => {
    ...
    socket.on('play', ({ room, cards, cardIndices }) => {
        var _a;
        if (isRateLimited(socket.id, 80))
            return;
        const upperRoom = ((_a = room === null || room === void 0 ? void 0 : room.toUpperCase) === null || _a === void 0 ? void 0 : _a.call(room)) || room;
    ...
    const handleActionEvent = ({ room, action }) => {
        var _a, _b, _c;
        const upperRoom = ((_a = room === null || room === void 0 ? void 0 : room.toUpperCase) === null || _a === void 0 ? void 0 : _a.call(room)) || room;
        const r = rooms.get(upperRoom);
        if (!r || !r.gameState)
        ...
        try {
            ...
            if (action.type === 'returnTribute') {
                nextState = (0, game_1.resolveReturnTribute)(r.gameState, pIdx, action.cards);
            }
            else {
                if (r.gameState.currentPlayer !== pIdx) {
                ...
                nextState = (0, game_1.playTurn)(r.gameState, action);
            }
            ...
            if (action.type === 'play') {
                const cards = (_b = action.cards) !== null && _b !== void 0 ? _b : [];
                ...
                if (pat) {
                    ...
                    emitSfx(r, {
                        ...
                        isKingBomb: !!((_c = pat.extra) === null || _c === void 0 ? void 0 : _c.isKingBomb),
                        count: cards.length,
                        hasJoker,
                        hasA2,
                    });
            ...
            if (nextState.finishedOrder.length >= nextState.playerCount) {
                ...
                if (r.matchHistory.length > 10)
                    r.matchHistory.shift();
                r.lastRoundResult = {
                    finishedOrder: nextState.finishedOrder,
                    revolution: nextState.revolution
                };
    ...
    socket.on('debug_win', ({ room }) => {
        ...
        try {
            const prevFinishedCount = r.gameState.finishedOrder.length;
            const nextState = (0, game_1.forceWin)(r.gameState, pIdx);
            r.gameState = nextState;
    ...
    // 聊天消息处理
    socket.on('chatMessage', ({ room, message, isEmoji }) => {
        if (isRateLimited(socket.id, 100))
            return;
        const r = rooms.get(room);
        if (!r)
